...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function handleFile(file) {
        ...
        try {
            if (file.name.match(/\.(mid|midi)$/i)) {
                ...
                setProgress(18, "오디오 최적화 중 (16kHz 리샘플링)...");
                ...
                setProgress(20, `변환 시작... (${formatTime(effectiveStart)} ~ ${formatTime(effectiveEnd)})`);
                ...
                for (let i = 0; i < totalChunks; i++) {
                    ...
                    setProgress(percent, `변환 중... (${i + 1}/${totalChunks} 구간 완료)`);
                }

                setProgress(100, "변환 완료! 마무리 중...");
        ...
        tracks.forEach((track, index) => {
            ...
            div.innerHTML = `
                ...
                <select data-track-index="${index}">
                    <option value="include" selected>변환 포함</option>
    ...
    convertBtn.addEventListener('click', () => {
        ...
        // Use user BPM if it's an audio file (AI Transcribed) or if user changed it?
        // For now, let's prioritize the input if it's visible/editable.
        // Actually, for MIDI files, we usually want the MIDI's BPM.
        // But the user asked for "BPM Input Field" specifically for "Audio transcription".
        // Let's use userBpm if the source is "AI Transcribed Piano", otherwise default to midiBpm but allow override?
        // Simple approach: Always use the input value. I'll pre-fill the input with MIDI BPM when loaded.
        const bpm = userBpm;
        ...
        if (noteCount > 0) {
            ...
            if (avgErrorMs > 30) {
                ...
                errorDisplay.textContent = `⚠️ 평균 박자 오차: ${avgErrorMs.toFixed(1)}ms (BPM이 맞지 않을 수 있습니다)`;
            } else {
                errorDisplay.style.display = 'block';
                errorDisplay.style.color = '#4caf50'; // Green
                errorDisplay.textContent = `✅ 평균 박자 오차: ${avgErrorMs.toFixed(1)}ms (양호)`;
            }
        } else {
